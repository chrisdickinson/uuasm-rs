...
impl Value {
    ...
    pub(crate) fn bit_eq(&self, rhs: &Self) -> anyhow::Result<()> {
        let (lbits, rbits) = match (self, rhs) {
            (Self::F32(l0f), Self::F32(r0f)) => (l0f.to_bits() as u64, r0f.to_bits() as u64),
            (Self::F64(l0f), Self::F64(r0f)) => (l0f.to_bits(), r0f.to_bits()),
            ...
            (Self::F64(l0f), Self::F64ArithmeticNaN) => {
                let l0 = l0f.to_bits();
                if (l0 & 0x7ff0_0000_0000_0000) == 0x7ff0_0000_0000_0000
                    && (l0 & 0x0008_0000_0000_0000) == 0x0008_0000_0000_0000
                {
                    return Ok(());
                } else {
