...
use crate::{nodes::Module, rt::{Machine, Value}};
...
pub(crate) fn assert_return<'a>(
    ...
    let returned = machine.call(field, args)
        .map_err(|e| anyhow::anyhow!(r#"failed to call "{field}" ({location}); error="{e:?}""#))?;
    assert_eq!(returned.len(), expected.len(), r#"result mismatch ({location}); returned={returned:?}; expected={expected:?}"#);
